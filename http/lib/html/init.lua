...
local opts = {} -- needed for silent/noerr/noout/nonl directives, also needed to be defined before `require` in such case
...
local function parse(text, limit, page_url) -- {{{
    local opts =
    ...
    if not opts.keep_danger_placeholders then -- {{{ little speedup by cost of potential parsing breakages
            ...
            if not (text:match(cc)) then -- {{{
                if not (tpr["<"]) or not (tpr[">"]) then -- {{{
                    if not (busy[i]) then -- {{{
                        ...
                        busy[i] = true
                        dbg("c:{%s}||cc:{%d}||tpr[c]:{%s}", str(c), cc:byte(),
                            str(tpr[c]))
                    ...
                    dbg("c:{%s}||cc:{%d}||tpr[c]:{%s}", c, cc:byte(),
                        str(tpr[c]))
                    dbg("%s", str(busy[i]))
                else -- if < or >
        ...
        if not (tpr["<"]) or not (tpr[">"]) then
            ...
            dbg("[FindPH]:#LINE# Found! || '<'=%d, '>'=%d", tpr["<"]:byte(),
                tpr[">"]:byte())
        end
        ...
        local function g(id, ...)
            ...
            dbg("[g]:#LINE# called, id: %s, arg[id]: %s, args { " ..
                    (("{%s}, "):rep(#arg):gsub(", $", "")) .. " }", id, arg[id],
                ...)
        ...
        text = text:gsub(
            ...
            "(%b'')", function(...) return g(2, ...) end):gsub(
                   "(=[%s]-)" .. -- same for "
            '(%b"")', function(...) return g(2, ...) end) -- Escape "<"/">" inside attr.values (see issue #50)
        :gsub("(<" .. -- Match "<",
    ...
    while dont_crash() do -- MainLoop {{{
        ...
        openstart, tpos, name = root._text:find(
            ...
            tpos)
        dbg("[MainLoop]:#LINE# openstart=%s || tpos=%s || name=%s",
            str(openstart), str(tpos), str(name))
        -- }}}
        if not name then break end
        -- Some more vars {{{
        index = index + 1
        local tag = ElementNode:new(index, str(name), (node or {}), descend,
                                    openstart, tpos)
        tag.page_url = page_url
        node = tag
        local tagloop
        ...
        while dont_crash() do -- TagLoop {{{
            ...
            start, apos, k, zsp, eq, zsp, quote =
                tagst:find("%s+" .. -- some uncaptured space
            ...
            dbg(
                ...
                str(start), str(apos), str(k), str(zsp), str(eq), str(quote))
            -- }}}
            ...
            if eq == "=" then
                ...
                dbg(
                    ...
                    str(start), str(apos), str(v), str(pattern))
            end
            -- }}}
            v = v or ""
            if tpl then -- {{{
        ...
        while dont_crash() do -- TagCloseLoop {{{
            ...
            dbg(
                ...
                str(closestart), str(closeend), str(closing), str(closename))
    ...
    if tpl then -- {{{
        dbg("tpl")
        for k, v in pairs(tpr) do root._text = root._text:gsub(v, k) end
    end -- }}}
    return root
end -- }}}
